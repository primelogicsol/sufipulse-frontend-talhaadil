...
export default function MyKalams() {
  ...
  const navigation = [
    { name: "Dashboard", href: "/writer", icon: User, current: false },
    { name: "Submit Kalam", href: "/writer/submit", icon: PenTool, current: false },
    { name: "My Kalams", href: "/writer/kalams", icon: BookOpen, current: true },
  ]
  ...
  const fetchKalams = async () => {
    try {
      const response = await fetch("https://sufi-sigma.vercel.app/kalams/writer/my-kalams")
      if (response.ok) {
        const data = await response.json()
        setKalams(data.kalams || [])
      }
    } catch (error) {
